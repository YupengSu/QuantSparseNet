...
def prune_wanda(args, model, tokenizer, device=torch.device("cuda:0"), prune_map=None):
    ...
    for i in range(len(layers)):
        ...
        if f"model.layers.{i}" in model.hf_device_map:   ## handle the case for llama-30B and llama-65B, when the device map has multiple GPUs;
            ...
            inps, outs, attention_mask, rotary_pos_emb = inps.to(dev), outs.to(dev), attention_mask.to(dev), rotary_pos_emb.to(dev)
        ...
        for j in range(args.nsamples):
            with torch.no_grad():
                outs[j] = layer(inps[j].unsqueeze(0), attention_mask=attention_mask, rotary_pos_emb=rotary_pos_emb)[0]
        for h in handles:
            h.remove()
